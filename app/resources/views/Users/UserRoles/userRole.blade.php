							...
							<div class="card-body">
								<div class="row justify-content-center">
									<div class="col-sm-4">
										<div class="form-group">
											...
											<input class="form-control" id="txtRoleName" placeholder="Role Name" type="text"
									...
									<thead>
										...
										<tr style="display: none;">
											...
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkAddAll" type="checkbox"><label for="chkAddAll"></label>
												</div>
											</th>
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkViewAll" type="checkbox"><label for="chkViewAll"></label>
												</div>
											</th>
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkEditAll" type="checkbox"><label for="chkEditAll"></label>
												</div>
											</th>
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkDeleteAll" type="checkbox"><label
														for="chkDeleteAll"></label></div>
											</th>
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkCopyAll" type="checkbox"><label for="chkCopyAll"></label>
												</div>
											</th>
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkExcelAll" type="checkbox"><label for="chkExcelAll"></label>
												</div>
											</th>
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkCSVAll" type="checkbox"><label for="chkCSVAll"></label>
												</div>
											</th>
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkPrintAll" type="checkbox"><label for="chkPrintAll"></label>
												</div>
											</th>
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkPDFAll" type="checkbox"><label for="chkPDFAll"></label>
												</div>
											</th>
											<th class="text-center">
												<div class="checkbox checkbox-dark"><input id="chkApprovalAll" type="checkbox"><label
														for="chkApprovalAll"></label></div>
											</th>
										</tr>
									</thead>
									<tbody id="tblroles">
								...
								<div class="row  mt-20 ml-20">
									<div class="col-sm-12 text-right">
										@if ($crud['view'] == 1)
											<a class="btn btn-sm  btn-outline-light  active txt-dark mr-10"
												href="{{ url('/') }}/users-and-permissions/user-roles/" id="btnCancel">Cancel</a>
										@endif
										<button class="btn btn-sm btn-outline-success btn-air-success" id="btnSubmit" type="button">Save</button>
									</div>
								</div>
							</div>
						</div>
		...
		$(document).ready(function() {
			...
			$('#btnCancel').click(() => {
				if (blnProcessing == false) {
					@if ($crud['view'] == 1)
						window.location.replace("{{ url('/') }}/users-and-permissions/user-roles/view");
					@endif
				} else {
			...
			$('#chkApprovalAll').click(function() {
				if ($('#chkApprovalAll').prop('checked') == true) {
					$('.chkApprovalAll:not(:disabled)').prop('checked', true);
				} else {
					$('.chkApprovalAll:not(:disabled)').prop('checked', true);
				}
			});
			...
			$(document).on('click', '#btnSubmit', async function() {
				...
				if (Status == true) {
					@if ($isEdit == true)
						var submiturl =
							"{{ url('/') }}/users-and-permissions/user-roles/edit/{{ $EditData[0]->RoleID }}";
					@else
						var submiturl =
							"{{ url('/') }}/users-and-permissions/user-roles/create";
					@endif
					swal({
							title: "Are you sure?",
										...
										swal({
											...
											@if ($isEdit == true)
												window.location.replace(
													"{{ url('/') }}/users-and-permissions/user-roles/"
												);
											@else
												window.location
													.reload();
			...
			const EnableDisable_EditDelete = async (Slug, ParentSlug) => {
				...
				var ApprovalID = "chk" + Slug + "Approval";
				...
				var Approval_CRUD_Value = parseInt($('#' + ApprovalID).attr('data-crud-value'));
				...
				if ($('#' + ElemID).prop('checked') == true) {
					...
					if (Approval_CRUD_Value == 1) {
						$('#' + ApprovalID).removeAttr('disabled');
					}
					//Checked
					...
					if ($('#' + ApprovalID).attr('data-checked') == "1") {
						$('#' + ApprovalID).prop('checked', true);
					}
					...
					if ((ParentSlug != null) || (ParentSlug != "")) {
						...
						$('#chk' + ParentSlug + 'Approval').removeAttr('disabled');
					}
				} else {
					if (Edit_CRUD_Value == 1) {
					...
					if (Approval_CRUD_Value == 1) {
						$('#' + ApprovalID).attr('disabled', 'disabled');
						$('#' + ApprovalID).prop('checked', false);
					}
					if ((ParentSlug != null) || (ParentSlug != "")) {
						...
						$('#chk' + ParentSlug + 'Approval').attr('disabled', true);
					}
				}
			}
			const Get_User_Role_info = async () => {
				...
				@if ($isEdit == true)
					...
					xhttp.open("POST", "{{ url('/') }}/users-and-permissions/user-roles/json/" + RoleID,
						true);
					xhttp.send(formData);
			...
			const Get_User_Rights = async () => {
				...
				for (i = 2; i <= (parseInt(totrows) + 1); i++) {
					...
					//Add
					if (($('#chk' + slug + 'Add').attr('disabled') == undefined) && ($('#chk' + slug + 'Add')
					...
					//Excel
					if ((($('#chk' + slug + 'Excel').attr('disabled') == false) || ($('#chk' + slug + 'Excel')
							.attr('disabled') == undefined)) && ($('#chk' + slug + 'Excel').attr(
							'data-crud-value') == "1")) {
					...
					//Print
					if ((($('#chk' + slug + 'Print').attr('disabled') == false) || ($('#chk' + slug + 'Print')
							.attr('disabled') == undefined)) && ($('#chk' + slug + 'Print').attr(
							'data-crud-value') == "1")) {
					...
					if ((($('#chk' + slug + 'Restore').attr('disabled') == false) || ($('#chk' + slug +
							'Restore').attr('disabled') == undefined)) && ($('#chk' + slug + 'Restore').attr(
							'data-crud-value') == "1")) {
					...
					//Approval
					if ((($('#chk' + slug + 'Approval').attr('disabled') == false) || ($('#chk' + slug +
							'Approval').attr('disabled') == undefined)) && ($('#chk' + slug + 'Approval').attr(
							'data-crud-value') == "1")) {
						if ($('#chk' + slug + 'Approval').prop('checked') == true) {
							t['approval'] = 1;
						}
					}
					UserRoles[mid] = t;
			...
			const get_menus_data = async () => {
				...
				formData.append("_token", $('meta[name=_token]').attr('content'));
				var xhttp = new XMLHttpRequest();
				xhttp.open("POST", RootUrl + "get/getMenusData", true);
				xhttp.send(formData);
				xhttp.onreadystatechange = async function() {
					if (this.readyState == 4 && this.status == 200) {
						response = await JSON.parse(this.responseText);
					...
					if ((response[i].Slug != "logout") && (response[i].Slug != "password-change")) {
						if (response[i].hasSubMenu == 1) {
							...
							html +=
								...
								response[i].Slug + 'Add" type="checkbox" ><label for="chk' + response[i].Slug +
								'Add"></label></div></td>';
							html +=
							...
							html +=
								'<td class="text-center"><div class="checkbox checkbox-primary"><input type="checkbox" data-id="' +
								response[i].MID + '" data-slug="' + response[i].Slug +
								'" data-crud-value="1" data-crud="Approval" class="chkPDFAll chkClick" id="chk' +
								response[i].Slug + 'Approval" ><label for="chk' + response[i].Slug +
								'Approval"></label></div></td>';
							...
							if ((response[i].Slug == "change-password") || (response[i].Slug == "profile") || (
									response[i].Slug == "dashboard")) {
							...
							html += '<td data-role="menu" data-parant-id="' + ParentID +
								...
								'" data-slug="' + response[i].Slug + '" style="' + style + '">' + response[i]
								.MenuName + '</td>';
							...
							html +=
								...
								.Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' + ParentSlug +
								'" data-crud-value="' + response[i].Crud['Add'] +
							...
							html +=
								'<td class="text-center"><div class="checkbox checkbox-success"><input type="checkbox" ' +
								Dashboard + ' ' + disabled + ' data-id="' + response[i].MID + '" data-slug="' +
								response[i].Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' +
								ParentSlug + '" data-crud-value="' + response[i].Crud['View'] +
								'" data-crud="View" class="chkViewAll MenuClick chk' + ParentSlug +
								'View" id="chk' + response[i].Slug + 'View" ><label for="chk' + response[i]
								.Slug + 'View"></label></div></td>';
							...
							html +=
								...
								disabled + ' data-id="' + response[i].MID + '" data-slug="' + response[i]
								.Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' + ParentSlug +
								'" data-crud-value="' + response[i].Crud['Edit'] +
							...
							html +=
								...
								disabled + ' data-id="' + response[i].MID + '" data-slug="' + response[i]
								.Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' + ParentSlug +
								'" data-crud-value="' + response[i].Crud['Delete'] +
								'" data-crud="Delete" class="chkDeleteAll MenuClick chk' + ParentSlug +
								'Delete" id="chk' + response[i].Slug + 'Delete" ><label for="chk' + response[i]
								.Slug + 'Delete"></label></div></td>';
							...
							html +=
								...
								disabled + ' data-id="' + response[i].MID + '" data-slug="' + response[i]
								.Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' + ParentSlug +
								'" data-crud-value="' + response[i].Crud['Copy'] +
							...
							html +=
								...
								disabled + ' data-id="' + response[i].MID + '" data-slug="' + response[i]
								.Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' + ParentSlug +
								'" data-crud-value="' + response[i].Crud['Excel'] +
								'" data-crud="Excel" class="chkExcelAll MenuClick chk' + ParentSlug +
								'Excel" id="chk' + response[i].Slug + 'Excel" ><label for="chk' + response[i]
								.Slug + 'Excel"></label></div></td>';
							...
							html +=
								...
								disabled + ' data-id="' + response[i].MID + '" data-slug="' + response[i]
								.Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' + ParentSlug +
								'" data-crud-value="' + response[i].Crud['CSV'] +
							...
							html +=
								...
								disabled + ' data-id="' + response[i].MID + '" data-slug="' + response[i]
								.Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' + ParentSlug +
								'" data-crud-value="' + response[i].Crud['Print'] +
								'" data-crud="Print" class="chkPrintAll MenuClick chk' + ParentSlug +
								'Print" id="chk' + response[i].Slug + 'Print" ><label for="chk' + response[i]
								.Slug + 'Print"></label></div></td>';
							...
							html +=
								...
								disabled + ' data-id="' + response[i].MID + '" data-slug="' + response[i]
								.Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' + ParentSlug +
								'" data-crud-value="' + response[i].Crud['Restore'] +
								'" data-crud="Restore" class="chkRestoreAll MenuClick chk' + ParentSlug +
								'Restore" id="chk' + response[i].Slug + 'Restore" ><label for="chk' + response[
									i].Slug + 'Restore"></label></div></td>';

							if (response[i].Crud['Approval'] == 0) {
								disabled = "disabled"
							} else {
								disabled = "";
							}
							html +=
								...
								disabled + ' data-id="' + response[i].MID + '" data-slug="' + response[i]
								.Slug + '" data-parant-id="' + ParentID + '" data-parent-slug="' + ParentSlug +
								'" data-crud-value="' + response[i].Crud['Approval'] +
								'" data-crud="Approval" class="chkApprovalAll MenuClick chk' + ParentSlug +
								'Approval" id="chk' + response[i].Slug + 'Approval" ><label for="chk' +
								response[i].Slug + 'Approval"></label></div></td>';
