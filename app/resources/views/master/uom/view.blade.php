		...
		<div class="row">
			<div class="col-sm-12">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item">UnitMeasurment</li>
				</ol>
			</div>
		</div>
	</div>
						...
						<div class="card-header text-center">
							<div class="form-row align-items-center">
								...
								<div class="col-md-4 my-2 text-right text-md-right">
									@if($crud['restore']==1)
										<a href="{{ url('/') }}/master/uom/trash-view" class="btn  btn-outline-light btn-sm m-r-10" type="button" > Trash view </a>
									@endif
									@if($crud['add']==1)
										<a href="{{ url('/') }}/master/uom/create" class="btn  btn-outline-success btn-air-success btn-sm" type="button" >Create</a> <!-- full-right -->
									@endif
								</div>
							</div>
						</div>
    ...
    $(document).ready(function(){
        ...
        const LoadTable=async()=>{
			...
			$('#tblUOM').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": RootUrl+"master/uom/data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnEdit',function(){
			window.location.replace("{{url('/')}}/master/uom/edit/"+ $(this).attr('data-id'));
		});
