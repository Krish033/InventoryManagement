						...
						<div class="card-header text-center">
							<div class="form-row align-items-center">
								...
								<div class="col-md-4 my-2 text-right text-md-right">
									@if($crud['restore']==1)
									<a href="{{ url('/') }}/master/customer/trash-view"
										class="btn  btn-outline-light btn-sm m-r-10" type="button"> Trash view </a>
									@endif
									@if($crud['add']==1)
									<a href="{{ url('/') }}/master/customer/create"
	...
	$(document).ready(function(){
        ...
        const LoadTable=async()=>{
			...
			$('#tblcustomer').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": RootUrl+"master/Customer/data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnEdit',function(){
			window.location.replace("{{url('/')}}/master/customer/edit/"+ $(this).attr('data-id'));
		});
